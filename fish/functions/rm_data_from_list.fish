...
function rm_data_from_list_help
    echo "Usage: rm_data_from_list <LIST_FILE> <TARGET_DIR>"
    echo ""
    echo "Removes directories listed in a file."
    echo ""
    echo "Arguments:"
    ...
    echo ""
    echo "Example:"
    echo "  rm_data_from_list cleanup_list.txt /var/www/html"
end

function rm_data_from_list
    ...
    if test (count $paths_to_remove) -eq 0
        echo $YELLOW"No valid directories found to remove."$CLEAR
        return 0
    end

    # 6. Show summary and ask for confirmation
    echo ""
    echo $RED$BOLD"The following directories will be PERMANENTLY DELETED:"$CLEAR
    echo "----------------------------------------------------"
